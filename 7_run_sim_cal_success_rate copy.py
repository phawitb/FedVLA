...
# - Auto-detect whether checkpoint expects text (language) by checking state_dict keys.
# - If text is used, load text prompts from text_inputs.json and encode via CLIP text encoder.
# - Auto-infer seq_len (pos_emb length) to set n_tokens_text correctly.
#
# Output:
...
def load_text_inputs(path: str):
    ...
    if not os.path.exists(path):
        print(f"[warn] text_inputs not found: {path} -> will fallback to task name prompts")
        return {}
    with open(path, "r", encoding="utf-8") as f:
        obj = json.load(f)
    if not isinstance(obj, dict):
        print("[warn] text_inputs.json is not dict -> ignore")
        return {}
...
def ckpt_has_text(model_state: dict) -> bool:
    # if any key begins with "text_to_tokens." -> model expects language
    return any(k.startswith("text_to_tokens.") for k in model_state.keys())


def infer_n_tokens_text_from_ckpt(model_state: dict, n_tokens_vision: int, n_tokens_state: int) -> int:
    # pos_emb shape: (1, seq_len, d_model)
    if "pos_emb" not in model_state:
        return 0
    seq_len = int(model_state["pos_emb"].shape[1])
    n_tokens_text = seq_len - int(n_tokens_vision) - int(n_tokens_state)
    return max(0, int(n_tokens_text))


def build_hptlike_from_ckpt(ck: dict, vision_dim: int, state_dim: int, action_dim: int, device: torch.device):
    cfg = ck.get("cfg", {}) or {}
    ms = ck["model_state"]
    ...
    if has_text:
        # if cfg didn't store it, infer from pos_emb length
        if n_tokens_text <= 0:
            n_tokens_text = infer_n_tokens_text_from_ckpt(ms, n_tokens_vision, n_tokens_state)
        if n_tokens_text <= 0:
            # safe fallback
            n_tokens_text = 2
...
def build_fedvla_from_ckpt(ck: dict, vision_dim: int, state_dim: int, action_dim: int, device: torch.device):
    cfg = ck.get("cfg", {}) or {}
    ...
    # infer whether personal includes text by looking at one client's personal keys
    ...
    # infer n_tokens_text from pos_emb in personal_state if present
    n_tokens_text = int(cfg.get("n_tokens_text", 0))
    if has_text:
        if "pos_emb" in any_personal and n_tokens_text <= 0:
            seq_len = int(any_personal["pos_emb"].shape[1])
            n_tokens_text = max(0, seq_len - n_tokens_vision - n_tokens_state)
        if n_tokens_text <= 0:
            n_tokens_text = 2
...
# Evaluation core
# -------------------------
def choose_prompt(task_name: str, text_bank: dict):
...
def run_eval_for_model(
    ...
    for task_name in tasks:
        ...
        for ep in range(1, episodes + 1):
            ...
            # choose & encode text once per episode (if needed)
            tfeat = None
            prompt = None
            if model_uses_text:
                prompt = choose_prompt(task_name, text_bank)
                tfeat = text_to_tfeat(prompt, clip_model, clip_processor, device)
            ...
            if not vw.isOpened():
                raise RuntimeError(
                    ...
                    "Check your OpenCV build / codec support."
                )
            ...
            for t in range(1, max_steps + 1):
                ...
                with torch.no_grad():
                    if model_uses_text:
                        act = model_obj(vfeat, st, tfeat).detach().cpu().numpy()[0]
                    else:
            ...
            print(
                ...
                f"min_dist {ep_summaries[-1]['min_dist']} | prompt {prompt}"
            )
...
def main():
    ...
    ap.add_argument("--text_inputs", type=str, default="text_inputs.json", help="JSON mapping task_name -> list of prompts")
    ...
    # Load prepared task map
    task_name_to_id = {}
    if os.path.exists(args.prepared_path):
        data = np.load(args.prepared_path, allow_pickle=True)
        if "task_map_json" in data.files:
            task_name_to_id = json.loads(str(data["task_map_json"][0]))  # {task_name: id}
    ...
    # Load text bank
    text_bank = load_text_inputs(args.text_inputs)
