# 5_train_fedvla.py
#
# FedVLA (Dual-Gating MoE + EDA aggregation) training script  [WITH LANGUAGE]
# - Uses prepared/prepared_dataset.npz
#   required keys: v_feats, t_feats, states, actions, task_ids, train_idx, val_idx
#   optional keys: task_map_json, text_inputs_json
#
# - Same logging style as centralized/fedavg:
#     CSV per round + JSONL detailed client steps
# - Saves best_model.pt + last_model.pt
#
# Notes (important):
# - This version "adds language" exactly like your centralized/fedavg models:
#     input = (v_feats, t_feats, states) -> action
# - FedVLA behavior remains:
#     trunk (DGMoE blocks + trunk_norm) is GLOBAL and aggregated by EDA
#     stem+head (vision_to_tokens, text_to_tokens, state_to_tokens, pos_emb, head) are PERSONAL per client
# - No extra "personal-only fine-tune stage": personalization happens naturally because personal params
#   are trained locally and persisted per client across rounds (same as paper's spirit).
#
# Compatible with your updated 7_run_sim_cal_success_rate.py (auto-detects text_to_tokens / pos_emb).
...
class FedVLA_Model(nn.Module):
    ...
    def forward(self, v: torch.Tensor, t: torch.Tensor, s: torch.Tensor):
        ...
        x = torch.cat([vt, tt, st], dim=1)  # (B, seq_len, d_model)
        x = x + self.pos_emb
...
# layer-wise weighted average of trunk params based on expert selection similarity per layer
# =========================
def eda_aggregate_trunk(
    ...
    for l in range(L):
        ...
        if denom <= 0:
            ...
            # w(l,i) = sum_j s(i,j) / sum_{i,j} s(i,j)
...
def client_local_train(
    ...
    ds = TensorDataset(
        ...
        torch.from_numpy(T[idx_train].astype(np.float32)),
    ...
    for ep in range(1, cfg.local_epochs + 1):
        for step_in_epoch, (bv, bt, bs, ba) in enumerate(loader, start=1):
            ...
            pred, sel_mat, densities = model(bv, bt, bs)
            ...
            if cfg.log_every_steps and (local_step % cfg.log_every_steps == 0):
                append_jsonl(
                    jsonl_step,
                    {
                        ...
                        "v": tensor_stats(bv.detach()),
                        "t": tensor_stats(bt.detach()),
                        "s": tensor_stats(bs.detach()),
...
def main():
    ...
    required = ["v_feats", "t_feats", "states", "actions", "task_ids", "train_idx", "val_idx"]
    missing = [k for k in required if k not in data.files]
    if missing:
        raise KeyError(
            "prepared_dataset.npz missing keys: "
            + ", ".join(missing)
            + "\nTip: rerun 2_load_prepare_dataset.py to generate t_feats."
        )
    ...
    stats = {
        ...
        "task_map": task_map,
        "note": "LANGUAGE enabled (t_feats). DGMoE + EDA implemented. Aggregate trunk only; stem+head personalized.",
    }
    with open(os.path.join(out_dir, "stats.json"), "w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)

    # Template model for init
    ...
    for r in range(1, cfg.rounds + 1):
        ...
        # Evaluate: per-client val (with its own personal + global trunk), then average
        ...
        for cid in clients:
            ...
            ds_val = TensorDataset(
                ...
                torch.from_numpy(T[idxv].astype(np.float32)),
